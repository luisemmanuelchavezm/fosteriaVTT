...
import {
  ...
  SummaryChoiceCard,
  TraitAccordion,
} from "./race/RaceSelectionBlocks";
import {
  buildInitialChoiceState,
  preserveChoiceState,
  removeChoiceDrivenItems,
  splitCompetencyItems,
  splitRaceChoices,
} from "./race/raceSelectionUtils";
import type {
  DndRaceDetail,
  DndRaceSummary,
  ...
  useEffect(() => {
    ...
    const loadRaceDetail = async () => {
      try {
        ...
        if ((error as Error).name !== "AbortError") {
          setSelectedRace(null);
          setRaceDetailError("No se pudo cargar la información de la raza.");
          setSelectedChoices({});
        }
      } finally {
        if (!abortController.signal.aborted) {
      ...
      >
        ...
        {selectedRace ? (
          ...
          <div className="mt-8 rounded-[24px] border border-dashed border-stone-300/15 bg-black/20 p-8 text-center text-sm text-stone-300">
            Selecciona una raza en el desplegable para ver su información, sus
